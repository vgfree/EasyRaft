...
struct eraft_taskis_group_add *eraft_taskis_group_add_make(struct eraft_group *group, ERAFT_DOTASK_FCB _fcb, void *_usr)
{
	...
	eraft_dotask_init(&object->base, ERAFT_TASK_GROUP_ADD, group->identity, _fcb, _usr);
...
struct eraft_taskis_entry_send *eraft_taskis_entry_send_make(char *identity, ERAFT_DOTASK_FCB _fcb, void *_usr, msg_entry_t *entry, struct etask *etask)
{
	struct eraft_taskis_entry_send *object = calloc(1, sizeof(*object));

	eraft_dotask_init(&object->base, ERAFT_TASK_ENTRY_SEND, identity, _fcb, _usr);
